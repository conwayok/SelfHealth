  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    TimerTask steps =
        new TimerTask() {
          ...
          public void run() {
            try {
              if (GoogleFitOauth.hasOauth(DailyDataNotificationService.this)
                  && settingsDao.getSettings().isConnectedToGoogleFit()) {
                GoogleFitRecording.fitRecordingSubscribe(
                    DailyDataNotificationService.this, DataType.TYPE_STEP_COUNT_DELTA);
                GoogleFitRecording.fitRecordingSubscribe(
                    DailyDataNotificationService.this, DataType.TYPE_CALORIES_EXPENDED);
                GoogleFitRecording.fitRecordingSubscribe(
                    DailyDataNotificationService.this, DataType.AGGREGATE_DISTANCE_DELTA);
                ...
                final GoogleFitHistory history =
                    new GoogleFitHistory(DailyDataNotificationService.this);
                history
                    .readHistoryData(startTime, endTime, 1, TimeUnit.DAYS)
                          ...
                          public void onSuccess(DataReadResponse dataReadResponse) {
                            ...
                            for (DataSet dataSet : datasets) {
                              for (DataPoint dp : dataSet.getDataPoints()) {
                                if (dp.getDataType().equals(DataType.AGGREGATE_STEP_COUNT_DELTA)) {
                                  for (Field field : dp.getDataType().getFields()) {
                                    ...
                                    DailyDataModel dailyDataModel =
                                        healthDataDao.getDailyData(LocalDate.now());
                                ...
                                //                                        for (Field field :
                                // dp.getDataType().getFields()) {
                                //                                            Log.i(TAG, "\tField: "
                                // + field.getName() + " Value: " + dp.getValue(field));
                                //                                        }
                              }
                            }
                          }
